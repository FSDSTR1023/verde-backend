...
import { newJWT, validateJWT } from "../helpers/JWT.helper.js";
...
export class Photographer {
  ...
  static edit = async (req, res) => {
    ...
    try {
      const { id: tokenId } = validateJWT(auth);

      if (id !== tokenId) {
        // TODO: Añadir validación de role
        return res.status(401).json({
          ok: false,
      ...
      const putPhotographer = await PhotographerModel.findByIdAndUpdate(
        id,
        { name, surname, email },
        { new: true }
      ); //! necesitas solo la línea de arriba
